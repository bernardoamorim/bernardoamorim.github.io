				...
				$remove_link = $excellent_settings['excellent_slider_link'];
					...
					if ($excerpt != '') {
						...
						if(!empty($slider_custom_text)){
							$excellent_category_sliders_display 	.= '<a title="'.esc_attr($slider_custom_text).'"' .' href="'.esc_url($slider_custom_url). '"'. ' class="btn-default vivid" target="_blank">'.esc_html($slider_custom_text). '</a>';
						}
						if($excellent_settings['excellent_slider_button'] == 0){
							...
							if($excerpt_text == '' || $excerpt_text == 'Continue Reading') :
								...
								$excellent_category_sliders_display 	.= '<a title='.'"'.the_title_attribute('echo=0'). '"'. ' '.'href="'.esc_url(get_permalink()).'"'.' class="btn-default dark">'.esc_html($excellent_settings[ 'excellent_tag_text' ]).'</a>';
							endif;
						}
